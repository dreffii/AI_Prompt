...
page = st.sidebar.radio("Go to:", ["Architectural Render Prompt", "Concept Building Prompt"])
...
if page == "Architectural Render Prompt":
    st.title("Architectural Render Prompt Generator 🏗️")
    ...
    if st.button("Generate Render Prompt"):
...
# ============================================================
# PAGE 2: Concept Building Prompt
# ============================================================
elif page == "Concept Building Prompt":
    st.title("Concept Building Prompt Generator 🧠")

    # --- Dropdowns ---
    building_styles = ["Modern Minimalist", "Brutalist", "Parametric", "Neo-Futuristic", "Biophilic", 
                       "Postmodern", "Industrial Loft", "Khmer Traditional", "Tropical Modern", "Classic Villa",
                       "Organic Architecture", "Metaverse Structure", "Sci-Fi Skyscraper", "Floating Architecture",
                       "Cultural Pavilion", "Sustainable Smart Building"]

    design_focus = ["Exterior Form", "Interior Space", "Urban Integration", "Landscape Interaction", 
                    "Structure & Frame", "Material Concept", "Lighting Concept", "Environmental Adaptation"]

    mood = ["Calm", "Dynamic", "Spiritual", "Innovative", "Minimal", "Organic", "Futuristic", "Cultural"]

    col1, col2 = st.columns(2)
    with col1:
        bstyle = st.selectbox("Building Style", building_styles)
        dfocus = st.selectbox("Design Focus", design_focus)
    with col2:
        bmood = st.selectbox("Mood / Concept Tone", mood)
        creativity = st.slider("Creativity Level", 0, 100, 70)

    keywords = st.text_input("Additional Keywords (optional)", placeholder="e.g. bamboo, concrete, curved roof, open courtyard")

    if st.button("Generate Concept Prompt"):
        prompt = f"Generate a new conceptual architecture design.\n"
        prompt += f"Building Style: {bstyle}.\n"
        prompt += f"Design Focus: {dfocus}.\n"
        prompt += f"Mood / Concept Tone: {bmood}.\n"
        prompt += f"Creativity Level: {creativity}%.\n"
        if keywords:
            prompt += f"Include these keywords or ideas: {keywords}.\n"
        prompt += "Describe a visionary yet buildable architectural concept with strong spatial logic and expressive form.\n"
        prompt += "Focus on proportion, balance, and cultural or environmental meaning."
